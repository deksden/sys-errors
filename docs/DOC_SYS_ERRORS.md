# Подсистема обработки ошибок (docs/errors/DOC_SYS_ERRORS.md, v0.1.4)
...
Проверяет принадлежность ошибки к определенному типу.

```javascript
function isError(
  error,           // Проверяемая ошибка
  errorDefinition  // Определение ошибки для проверки
)
```
